...
public class TreeSet<AnyType> extends AbstractCollection<AnyType> implements SortedSet<AnyType>
{
  ...
  /**
   * Finds the greatest element that is less than or equal to target.
   * 
   * @param the_target the target floor.
   * @return greatest element in the set less than or equal to target,
   * null if it doesn't exist.
   */
  public AnyType floor(final AnyType the_target)
  {
    AnyType floor = null;
    AnyType next = null;
    final TreeSetIterator itr = new TreeSetIterator();
    
    while (itr.hasNext())
    {
      next = itr.next();
      
      // test if element in next node is smaller than target
      if (compare(the_target, next) > 0)
      {
        floor = next;
      }
    }
  ...
  /**
   * Finds the smallest element that is more than or equal to target.
   * 
   * @param the_target the target ceiling.
   * @return smallest element in the set greater than or equal to target,
   * null if it doesn't exist.
   */
  public AnyType ceiling(final AnyType the_target)
  {
    AnyType ceiling = null;
    AnyType next = null;
    int found = 0;
    final TreeSetIterator itr = new TreeSetIterator();
    ...
    while (itr.hasNext())
    {
      ...
      // test if element in next node is larger than target and if ceiling is found
      if (compare(the_target, next) < 0 && found != 1)
      {
        ceiling = next;
        found = 1;
      }
